...
export default function LearnPage() {
	const t = useTranslations("WordlistLearn");
	...
	return (
		<main className="min-h-[calc(100vh-80px)] bg-[#030616] px-4 py-8 text-white sm:px-6 sm:py-12">
			<div className="mx-auto w-full max-w-6xl">
				...
				<section className="mt-10 grid gap-4 md:grid-cols-3" aria-label={t("modesLabel")}>
						...
						return (
							...
							>
								...
								<div className="relative flex min-h-64 flex-col">
									...
									<h2 className="mt-7 text-2xl font-bold">{t(`modes.${mode.id}.title`)}</h2>
									...
									<div className="mt-5 flex items-center gap-4 border-t border-slate-800 pt-4 text-xs text-slate-500">
										<span>{t(`modes.${mode.id}.detail`)}</span>
										<span className="inline-flex items-center gap-1.5">
											<Clock3 className="h-3.5 w-3.5" />
											{t(`modes.${mode.id}.time`)}
										</span>
									</div>
